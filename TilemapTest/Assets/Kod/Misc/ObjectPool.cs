...
namespace Misc
{
    ...
    public class ObjectPool<T> : IObjectPool<T> where T : class, new()
    {
        ...
        public void Resize(int size)
        {
            Debug.LogWarning($"Resize:{typeof(T)}, Newsize:{size}");
            try
            {
                lock (_lock)
                {
            ...
            {
                Debug.LogError($"Failed To Resize: {e}");
                throw;
            }
        }
        ...
        public T GetNext()
        {
            ...
            {
                Debug.LogError($"Failed To GetNext: {e}");
                if (_alloc == null)
                {
                    return new T();
                }
        ...
        public void Release(T obj)
        {
            ...
            {
                Debug.LogError($"Failed To Release: {e}");
            }
        }
        ...
        public bool Recycle(T obj)
        {
            ...
            {
                Debug.LogError($"Failed To Recycle: {e}");
                return false;
            }
        }
